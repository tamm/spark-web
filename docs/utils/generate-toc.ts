...
export function generateRowToc(content: string): HeadingDataRaw[] {
  ...
  if (headings.length) {
    toc = headings.map((heading: Record<number, any>) => {
      ...
      const headingSlug = toSlug(headingText);
...
export type HeadingData = HeadingDataRaw & {
  items: HeadingData[];
  id: string;
};

export const generateToc = (content: string): HeadingData[] => {
